...
let copyroute route =
  Array.map (fun (t,(x,y)) ->(t,(x,y))) route

let copytraj route =
  Array.map (fun (l,mi,ma) ->(List.map (fun (a,b,c) -> (a,b,c)) l,mi,ma)) route
      

let copyacft a =
  let (man1,man2,man3)=a.man in
  {speed=a.speed; route=copyroute a.route; predict=copyroute a.predict; dev=copyroute a.dev; 
   man=(man1,man2,man3); traj=copytraj a.traj}

let printdev pb =
  Array.iteri (fun i a -> 
    Printf.printf "\navion %d: " i;
    Array.iter (fun (t,(x,y)) -> Printf.printf "%f (%f;%f)" t x y) a.dev;
    Printf.printf "\n") pb
...
let incert_route cfg =
  Array.map (TP.convert cfg) [|0; 1; 8; 9|]
let incert_man cfg = 
  Array.map (TP.convert cfg) (Array.init 16 (fun i -> 2 * i))
let incert_all cfg =
  Array.map (TP.convert cfg) (Array.init 64 (fun i -> i))

let set_traj_cfg a cfg =
  let to_xyz (_, (x, y)) = (x, y, 0.) in
  let l = Array.length a.dev in
  if l<2 then a.traj <-Array.init cfg.TP.nb_steps (fun _ -> ([],0.,0.))
  else
    begin
	...
	{TP.t0=max 0. (t0 -. t); TP.t1=t1 -. t; TP.a=alpha; TP.fl=0.; TP.n=n} in
      let incert = (if t0 = t1 then incert_route else incert_all) cfg in
...
let set_traj a err =
  set_traj_cfg a (cfg err)
(*
let set_traj_par a errs =
  let cfg = {(cfg 1) with 
    TP.errspeed = errs.(0); 
    TP.errt0 = errs.(1);
    TP.errt1 = errs.(2);
...
let set_man err a = set_man_cfg a (cfg err)
(*
let set_man_par a errs =
  let cfg = {(cfg 1) with 
    TP.errspeed = errs.(0); 
    TP.errt0 = errs.(1);
    TP.errt1 = errs.(2);
...
let detect_traj err acft i =
  ...
  Array.iter (fun a ->
    if Array.length (get_traj a) = 0 then set_traj a err) acft;
...
let measurelength trajs=
  let t=Array.map (fun _ -> 0) trajs in
  Array.iteri (fun i traj ->
    Array.iter (fun (l,mi,ma) -> if l<>[] then t.(i) <- t.(i)+1) traj) trajs;
  Array.iter (fun k -> Printf.printf "%d " k) t;
  print_newline ();
  flush stdout;
  t

    
let detect_traj_all err acft =
  let cfg = {(cfg err) with TP.nb_ac = Array.length acft} in
  let trajs = Array.map get_traj acft in
(*  measurelength trajs; *)
  Array.iteri (fun i traj ->
  ...
  Array.iteri (fun i ai ->
	...
	List.iter (fun t ->
	  ...
	  if TP.all_pairs cfg.TP.norm2 traj_i traj_j false then
	    begin
	      let (ti,_,_)=trajs.(i).(t)
	      and (tj,_,_)=trajs.(j).(t) in
	      let li= List.map (fun (x,y,_) -> (x,y)) ti
	      and lj= List.map (fun (x,y,_) -> (x,y)) tj in 
	      lines.(i) <- (j,li)::lines.(i);
	      lines.(j) <- (i,lj)::lines.(j);
	    end)
	  (TP.prefiltre cfg mmtab.(i) mmtab.(j))))
      acft) acft;
  lines
      
let detect_traj_all err = safe "detect_traj_all" (detect_traj_all err)
...
let detect_traj_mat err acft =
  ...
  Array.iteri (fun i traj ->
    if Array.length traj = 0 then 
      trajs.(i) <- Array.create cfg.TP.nb_steps ([], 0., 0.)) trajs;
  let mmtab = TP.minmax cfg trajs in
  let mat = Array.init cfg.TP.nb_ac (fun _ -> Array.init cfg.TP.nb_ac (fun _ ->0.)) in
  let conf = Array.init cfg.TP.nb_ac (fun _ -> 0.) in
  let tot=ref 0. in
  Array.iteri (fun i ai ->
    Array.iteri (fun j aj ->
...
let detect_traj_mat err = safe "detect_traj_mat" (detect_traj_mat err)

let detect_traj_part err acft lij=
  let cfg = {(cfg err) with TP.nb_ac = Array.length acft} in
  let trajs = Array.map get_traj acft in
  Array.iteri (fun i traj ->
    if Array.length traj = 0 then 
  ...
  List.iter (fun (i,j) ->
	...
	(mat.(i).(j) <- mat.(i).(j)+.1.;
	 ...
	 conf.(j) <- conf.(j)+.1.;
	 tot:= !tot+.1.))
      (TP.prefiltre cfg mmtab.(i) mmtab.(j))) lij;
  (mat,conf, !tot)
    
let detect_traj_part err = safe "detect_traj_part" (detect_traj_part err)
...
let modifyfornico err tabacft tabman=
  let t = Array.mapi (fun i a ->
    let (t0,h,t1)=tabman.(i) in
    turn tabacft.(i) t0 h t1;
    set_man err tabacft.(i); i+10) tabacft in t
...
let random size n =
  ...
  let acft = Array.init n (fun i -> rnd_acft alpha.(i) i) in
  Array.iteri (fun i _ ->
    while first_conf_t acft i < fst acft.(i).route.(0) +. min_conf_t do
      acft.(i) <- (rnd_acft alpha.(i) i) ;
    done) 
    acft;
  acft
...
let save filename acft err=
  ...
  Printf.fprintf file "%d %.4f %f %f %f %f\n" (Array.length acft) t
...
let load filename =
  ...
  Scanf.sscanf (input_line file) "%d %f %f %f %f %f" (fun n t incert0 incert1 incert2 incert3->
	...
	a)) in
    close_in file;
    ([|incert0;incert1;incert2;incert3|],acft))
