...
(* Définition d'un problème avec tmin le début, tmax la fin, t0min le début min de lmanoeuvre, pref pour dire si l'avion tournait à droite ou à gauche lors de la précédente optimisation acft les avions et incert le vecteur d'incertitudes*)    
...
(* definition d'une manoeuvre t0 début, t1 fin, h heading change*)
      
type man = {t0: float; t1: float; h: float}

(* type data de l'algo gen: avec data.d pour le tableau de manoeuvres, data.f pour le tableau des conflits de l'avion i,mat la matrice des conflits des avions i j , data.tot totalise le nombre de conflits data.t le tableau des avions (voir le type t dans acft.ml*)
type data =
    {
     d: man array;
     f: float array;
...
(*Mise à l'echelle: verification après croisement et mutation que les données sont dans les bornes autorisées*)    
let scale d =
...
(* comptage du nombre d'avions déviés*)    
let get_nb_dev data =
  Array.fold_left (fun n m -> 
    if m.t0 <> m.t1 then (n +. 1.) else n)
    0. data.d 
(* mesure du retard des avions *)
let get_delay acft =
  Array.fold_left (fun d a -> d +. Acft.delay a) 0. acft

(*fonction d'évaluation de l'algo gen*)    
let evalData data =
...
(* génère la population initiale de l'AG*)    
let genData () = 
...
(*distance pour le sharing, rien avoir avec la distance entre deux avions*)    
let dataDistance d1 d2 =
  let dist = ref 0 in
  Array.iteri (fun i _ -> if d1.d.(i).h *. d2.d.(i).h < 0. then incr dist) d1.d;
  float !dist

(*barycentre pour le sharing*)    
